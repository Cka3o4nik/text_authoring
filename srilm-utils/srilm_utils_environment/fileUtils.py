...
#		match_pattern = lambda patt, path: self.match_pattern(patt, path, flags)
		res = [self.__class__(path) for path in raw_list if re.search(pattern_RE, unicode(path))]
		return res


	def search(self, pattern, flags=0):
		logOut(u'Searching for "%s" in path [%s]...'%(pattern, self.path))
...
#		match_pattern = lambda patt, path: self.match_pattern(patt, path, flags)
		...
		logOut(u'Files found: %d'%len(res))
		return res
...
class FilePathNameEnc(FilePathNameBase):
	...
	def to_fs(self):
		#logDbg("self.path: %r"%self.path, logger_name=FILE_UTILS_LOGGER)
		return self.path.encode(strFS_ENCODING)#.encode(SYS_ENC) # .decode()
...
#		self.args.append(unicode(arg))
	...
	def launch(self, afWait=True, stdindata=None, stdout=None):
		arg_list = [self.path]
		arg_list.extend(unicode(el) for el in self.args) 
		return launch(arg_list, afWait, stdindata, stdout) 
#-------------------------------------------------------------------

ZIPPER_PATH = FilePathName(u'//192.168.77.2/public/shared_soft/7-zip/7za.exe')

def NTFilenameDecode(astrData): 
	return unicode(astrData, "CP1251")
...
def zip(aZipFilename, aFileList):
	...
	try:
		...
		map(zip_file.write, (autoconvert_str(f) for f in aFileList))
	except:
		logExc()
...
def launch(path, afWait=True, stdindata=None, stdout=None):
	strPath = str(path)
	cmd_type = type(path)
...
def recode_file(src_filename, src_enc, dst_filename, dst_enc):
	logOut(u'Creating %s version of %s...'%(dst_enc, src_enc))
...
def find_process_files(src_dir, filemask, proc_func, *pf_args, **pf_kwargs):
	...
	for i, src_file in enumerate(src_list):
		logOut(u'Processing source file [%s] %d/%d'%(src_file, i+1, src_list_len), logger_name = FILE_UTILS_LOGGER)
		args = list(pf_args)
		args.insert(0, i)
...
def find_files_cached_iter(cache_path, src_path, filemask, report_count=1):
	...
	logOut(u'Extracting file cache...', logger_name = FILE_UTILS_LOGGER)
	db_key = str(src_path)
	try:
...
#		logOut(u'Creating filter...', logger_name = FILE_UTILS_LOGGER)
		...
		filtered_list = it.ifilter(lambda x: x.match_pattern(filemask), map(autoconvert, raw_cached_list))
		logOut(u'Raw files to be processed: %d'%src_list_len, logger_name = FILE_UTILS_LOGGER)
#		pdb.set_trace()
...
#	pdb.set_trace()
	if src_list_len:
		...
		logOutLen(u'Processing new (not cached) source files', src_list, logger_name = FILE_UTILS_LOGGER)
		for i, src_file in enumerate(src_list):
			if (i+1)%report_count==0:
				logOut(u'Processing source file [%s] %d/%d'%(src_file, i+1, src_list_len), logger_name = FILE_UTILS_LOGGER)
			yield i, src_file
