...
import urllib, urllib2, re, shelve, random, time, datetime, zlib, bz2#, sys, string
...
def get_page_with_cookies(url, post_params = None, opener = None, proxies = None, user_agent = None, timeout = secHTTP_WAIT_TIMEOUT):
	...
	url_handlers = []
	if not opener:		
		url_handlers = [urllib2.HTTPCookieProcessor(), ]
#if proxies=='':
#			proxies = 

	if proxies:
#			url_handlers.append(urllib2.ProxyHandler({'http': 'http://'+proxy})) - previous version
		url_handlers.append(urllib2.ProxyHandler(proxies)) # Переделал для большей универсальности (SOCKS и др.)
#else:
#	url_handlers.append (urllib2.ProxyHandler({'http': 'http://'}))
	...
	if post_params:
		...
		logOut(u'Post params: %s'%post_params, logger_name = HTTP_LOGGER)
...
def post_params_to_str(post_params):
	if post_params:
		param_list = [key+'='+value for key, value in post_params.iteritems()]
...
class URL_Fetcher(BZ2_Cacher):
	...
	def build_opener(self):
			url_handlers = [urllib2.HTTPCookieProcessor(), ]
			if self.proxies:
				url_handlers.append(urllib2.ProxyHandler(self.proxies))

			self.opener = urllib2.build_opener(*url_handlers)
