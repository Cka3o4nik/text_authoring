...
def logDbg(astrMsg, logger_name = DEFAULT_LOGGER_NAME, caller=None):
	...
	logOut(caller+': '+astrMsg, DEBUG, logger_name)
...
def logFuncName(astrMsg='', level = INFO, logger_name = DEFAULT_LOGGER_NAME):
	logOut('%s%s'%(getCallerName(logger_name), astrMsg), level, logger_name)

def logErr(astrMsg, logger_name = DEFAULT_LOGGER_NAME):
	logOut('function %s: %s'%(getCallerName(logger_name), astrMsg), ERROR, logger_name)
...
def getCallerName(logger_name):
	return str(logging.getLogger(logger_name).findCaller())
